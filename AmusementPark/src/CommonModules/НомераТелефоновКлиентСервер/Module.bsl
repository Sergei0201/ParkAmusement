...
// Преобразует нормер телефонп из любого формата и нормальный для быстрого поиска
// Нормализованный телефон состоит из 11 цифор и начинается на 7.
// 
// Параметры:
...
Функция НормализованныйНомерТелефона (Телефон, ВызыватьИсключение = Ложь) Экспорт
	...
	Если СтрДлина(ЦифрыТелефона) <> 11 Тогда
		Ошибки.Добавить("Номер Телефона должна содержать 11 цифр"); 
	КонецЕсли;
